...
class LineIntensityProfileWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.inputRulerSelector.setToolTip( "Select the ruelr to collect intensity profile." )
  ...
  def onSelect(self):
    self.applyButton.enabled = self.inputSelector1.currentNode() and self.inputSelector2.currentNode() and self.inputRulerSelector.currentNode()
  ...
  def onApplyButton(self):
    ...
    logic.run(self.inputSelector1.currentNode(),
              ...
              self.inputRulerSelector.currentNode() )
...
class LineIntensityProfileLogic(ScriptedLoadableModuleLogic):
  ...
  def run(self,inputVolume1,inputVolume2, inputRuler):
    ...
    legendNames  = [inputVolume1.GetName()+' - '+inputRuler.GetName(),
                    inputVolume2.GetName()+' - '+inputRuler.GetName()]

    self.showChart(imageSamples,legendNames)

    return True

  def showChart(self,imageSamplesLists, legendNamesLists):
      lm = slicer.app.layoutManager()
      layoutNode=slicer.vtkMRMLLayoutNode()
      lm.setLayout(layoutNode.SlicerLayoutFourUpQuantitativeView)

      doubleArrays = []
      for sample in imageSamplesLists:
      ...
      for pairs in zip(legendNamesLists,doubleArrays):
  ...
  def probeVolume(self,volumeNode, rulerNode):
      ...
      scalarArray = probe.GetOutput().GetPointData().GetArray('ImageScalars')
      print(scalarArray)
      return scalarArray
